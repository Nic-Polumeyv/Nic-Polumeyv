<script lang="ts">
	...
	let isComplete = $derived(!loop && currentWordIndex === wordsToAnimate.length - 1 && currentCharIndex >= currentWordGraphemes.length && phase !== 'deleting');

	// Derived: Should show cursor
	let shouldShowCursor = $derived(showCursor && !isComplete && (hasMultipleWords || loop || currentCharIndex < currentWordGraphemes.length));
...
<motion.span bind:ref={elementRef} class={cn('leading-20 tracking-[-0.02em]', className)} {...props}>
	{displayedText}
	{#if shouldShowCursor}
